...
class JanusGCPOTrainer(Trainer):
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        if self.is_deepspeed_enabled and hasattr(self.model, 'global_steps'):
            latest_file = os.path.join(checkpoint_dir, "latest")
            if not os.path.exists(latest_file):

                import glob
                zero_dirs = glob.glob(os.path.join(checkpoint_dir, "global_step*"))
                if zero_dirs:
                    step_name = os.path.basename(max(zero_dirs, key=os.path.getmtime))
                    with open(latest_file, 'w') as f:
                        f.write(step_name)
                    print(f"[Save] Created missing 'latest' file for ZeRO-3 resume")
        
        print(f"[Save] Checkpoint saved at step {self.state.global_step}")
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # ------------------------------------------------------------
        # divergence switch: rkl / fkl / js
        # use ref_over_pi = pi_ref / pi = exp(ref_logp - logp)
        # ------------------------------------------------------------
        ...
        ref_over_pi = torch.exp(ref_minus_pi_log_ratio_clamped)  # pi_ref / pi
        ...
        if loss_divergence_flag == "rkl":
            ...
            per_token_div = ref_over_pi * ref_minus_pi_log_ratio_clamped - ref_over_pi + 1.0
        ...
        mean_kl = (per_token_div * completion_mask).sum() / completion_mask.sum()
