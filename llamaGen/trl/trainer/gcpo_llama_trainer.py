...
class GRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        # Trained model
        model_id = model
        dtype = {"bf16": torch.bfloat16, "fp16": torch.float16, "fp32": torch.float32}['bf16']
        # model = LlamaForCausalLM.from_pretrained(model, device_map="auto").to(dtype=dtype)
        model = LlamaForCausalLM.from_pretrained(model).to(dtype=dtype)
        ...
        if self.beta == 0.0:
            ...
            self.ref_model = LlamaForCausalLM.from_pretrained(model_id).to(dtype=dtype)
            ...
            print("create_reference_model")
            self.ref_model = create_reference_model(model)
            # self.ref_model.train()

        # Processing class
        if processing_class is None:
            processing_class = model.t5_tokenizer
    ...
    def _get_per_token_logps(self, model, input_ids, attention_mask, logits_to_keep):
        ...
        # 此处的 model 是 LlamaForCausalLM, 下面有t5_model和LlamaModel
        prompt_ids_length = input_ids.shape[1] - logits_to_keep
        prompt_ids = input_ids[:, :prompt_ids_length]
        completion_ids = input_ids[:, -logits_to_keep:]
        if isinstance(model, torch.nn.DataParallel) or isinstance(model, torch.nn.parallel.DistributedDataParallel):
            model = model.module
        prompt_embeds, completion_embeds = model.get_prompts_completion_embeddings(prompt_ids, completion_ids)
        prompt_masks = attention_mask[:, :prompt_ids_length]
        completion_masks = attention_mask[:, -logits_to_keep:]
        
        logits = model.get_logits_to_keep(prompt_embeds, prompt_masks, completion_embeds, completion_masks)
    ...
    def _generate_and_score_completions(
        ...
        if self.log_completions and self.state.global_step % self.args.logging_steps == 0:
            ...
            if self.accelerator.is_main_process:
                ...
                if self.args.report_to and "wandb" in self.args.report_to and wandb.run is not None:
                ...
                elif self.args.report_to and "swanlab" in self.args.report_to and swanlab.run is not None:
                    import pandas as pd
                    ...
                    df = pd.DataFrame(table)
                    swanlab.log({"completions": swanlab.Table(dataframe=df)})
    ...
    def select_high_entropy_gradient_tokens_batch(self, entropy_seq, topk_frac=0.1):
        """
        Args:
            entropy_seq: [B, N], N=h*w 或 N=patch_num
            h, w: 图像高宽
            topk_frac: 选取比例 (如0.1表示每张图选10%)
        Returns:
            selected_mask: [B, N]，每个batch的选中token为1
        """
        ...
        for b in range(B):
            grad_sub = grad_flat[b, offset:offset+N_valid]  
            k = int(N * topk_frac)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # Select divergence used in the loss regularizer
        # choices: "rkl", "fkl", "js"
        loss_divergence_flag = "fkl"
        print(f"==loss_divergence_flag:{loss_divergence_flag}==")

        if self.beta != 0.0:
            # log(pi_ref / pi)
            ref_minus_pi_log_ratio = ref_per_token_logps - per_token_logps
            # clamp for numerical stability
            ref_minus_pi_log_ratio_clamped = torch.clamp(ref_minus_pi_log_ratio, -20.0, 20.0)
            # w = pi_ref / pi
            ref_over_pi = torch.exp(ref_minus_pi_log_ratio_clamped)

            if loss_divergence_flag == "rkl":
                # D_KL(pi || pi_ref) under pi-measure
                per_token_div = ref_over_pi - ref_minus_pi_log_ratio_clamped - 1.0

            elif loss_divergence_flag == "fkl":
                # D_KL(pi_ref || pi) rewritten under pi-measure:
                # E_pi [ (pi_ref/pi) * log(pi_ref/pi) - (pi_ref/pi) + 1 ]
                per_token_div = (
                    ref_over_pi * ref_minus_pi_log_ratio_clamped
                    - ref_over_pi
                    + 1.0
                )

            elif loss_divergence_flag == "js":
                # JS(pi || pi_ref) under pi-measure, numerically stable form
                # w = pi_ref / pi
                # x = pi / pi_ref = 1 / w
                x = torch.exp(-ref_minus_pi_log_ratio_clamped)
                kl_pi_m = math.log(2.0) - torch.log1p(ref_over_pi)
                kl_ref_m_on_pi_measure = ref_over_pi * (math.log(2.0) - torch.log1p(x))
                per_token_div = 0.5 * (kl_pi_m + kl_ref_m_on_pi_measure)

            else:
                raise ValueError(f"Unknown loss_divergence_flag: {loss_divergence_flag}")

        ##### mask #####
        # top 10%
        seq_len = per_token_logps.shape[1]
        first_10_len = int(seq_len * 0.1)
        first_10_mask = torch.zeros_like(per_token_logps, dtype=bool)
        first_10_mask[:, :first_10_len] = True
        
        print(inputs["entropy"].shape)
        # entropy gradient
        entropy = inputs["entropy"]  # (B, L)
        entropy_mask = self.select_high_entropy_gradient_tokens_batch(entropy, topk_frac=0.1)  # (B, L)
        entropy_mask = entropy_mask.bool()
        
        # Similarity difference
        embeddings = inputs["embeddings"]  # (B, L, D)
        sqe_len = embeddings.size(1)
        embedding_size = embeddings.size(2)
        # print(embeddings.shape)
        embeddings = embeddings.view(-1, self.num_generations, sqe_len, embedding_size)
        # print(embeddings.shape)
        cos_sim_list = []
        for token_index in range(sqe_len):
            embedding = embeddings[:, :, token_index, :]  # [batch_size * num, embedding_size]
            cos_sim_matrix = F.cosine_similarity(
                embedding.unsqueeze(2),  # [batch_size, num, 1, embedding_size]
                embedding.unsqueeze(1),  # [batch_size, 1, num, embedding_size]
                dim=-1
            )
            mask = ~torch.eye(self.num_generations, dtype=bool, device=cos_sim_matrix.device)  # [num, num]
            avg_cos_sim = cos_sim_matrix[:, mask].mean(dim=1)
            avg_cos_sim = torch.repeat_interleave(avg_cos_sim, self.num_generations, dim=0)# [batch_size]    
        ...
        if self.beta != 0.0:
            per_token_loss = per_token_loss + self.beta * per_token_div
        loss = (per_token_loss * completion_mask).sum() / completion_mask.sum()
        # Log the metrics
        mode = "eval" if self.control.should_evaluate else "train"

        if self.beta != 0.0:
            mean_kl = ((per_token_div * completion_mask).sum(dim=1) / completion_mask.sum(dim=1)).mean()
