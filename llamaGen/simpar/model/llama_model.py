...
from simpar.model.llamagen.t5 import T5Embedder

import time
from simpar.model.drafters.kv_cache import initialize_past_key_values
...
class CaptionEmbedder(nn.Module):
    ...
    def token_drop(self, caption, force_drop_ids=None):
        ...
        if force_drop_ids is None:
            ...
            drop_ids = force_drop_ids == 1
        prompt_caption = caption[:, :self.cls_token_num]
        prompt_caption = torch.where(drop_ids[:, None, None], self.uncond_embedding, prompt_caption)
        caption = torch.cat([prompt_caption, caption[:, self.cls_token_num:]], dim=1)
        return caption
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def get_prompts_embeddings(self, input_ids):
        return self.cls_embedding(input_ids, train=self.training)[:, :self.cls_token_num]
    ...
    def forward(
        ...
        if (past_key_values is None or input_ids is None) and inputs_embeds is None:
        ...
        if attention_mask is None:
            ...
            if attention_mask.shape[1] > self.cls_token_num or 0 in attention_mask:
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        cache_dir = 'checkpoints Path'
        dir_or_name = 'models--google--flan-t5-xl'
        ...
        self.t5_tokenizer = T5Tokenizer.from_pretrained(os.path.join(cache_dir, dir_or_name), padding_side="left")
    ...
    def get_prompt_embeddings(self, prompt_ids):
        prompt_embeds = self.t5_model.get_embeddings_from_ids(prompt_ids)
        prompt_embeds = prompt_embeds.to(self.model.dtype)
        prompt_embeds = self.model.cls_embedding(prompt_embeds, train=self.model.training)
        return prompt_embeds
    
    def get_prompts_completion_embeddings(self, prompt_ids, completion_ids):
        
        # 返回的prompt_embeds是2048, completion_embeds是1280
        prompt_embeds = self.t5_model.get_embeddings_from_ids(prompt_ids)
        prompt_embeds = prompt_embeds.to(self.model.dtype)

        completion_embeds = self.model.embed_tokens(completion_ids)
    ...
    def get_logits_to_keep(self, caption_embs, emb_masks, completion_embs, completion_masks):
        new_emb_masks = torch.flip(emb_masks, dims=[-1])
        new_caption_embs = []
        # 将caption_embs中的caption_emb按照emb_mask中的1的位置进行翻转
        ...
        new_caption_embs = new_caption_embs * new_emb_masks[:,:, None]
        
        attention_mask = torch.cat([new_emb_masks, completion_masks], dim=1)
        
        caption_embs = self.model.cls_embedding(new_caption_embs, train=self.model.training)
        completion_embs = completion_embs
        attention_mask = attention_mask

        cond_combined = torch.cat([caption_embs, completion_embs], dim=1).to(self.dtype)

        output = self.forward(inputs_embeds=cond_combined, attention_mask=attention_mask)
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        self.cfg = cfg
        prompt_ids, prompt_mask, caption_embs, emb_masks = self.t5_model.get_text_embeddings(prompt)
        new_emb_masks = torch.flip(emb_masks, dims=[-1])
        new_caption_embs = []
        # 将caption_embs中的caption_emb按照emb_mask中的1的位置进行翻转
        for idx, (caption_emb, emb_mask) in enumerate(zip(caption_embs, emb_masks)):
        ...
        if hasattr(self.model, "past_key_values"):
            ...
            (
                ...
                current_length_data,
            ) = initialize_past_key_values(self.model, bs)
        ...
        # 如果执行了cfg，则需要将c_indices和cond_null拼接
        if cfg is not None:
        ...
        next_token, next_token_prob, next_token_logits = sample(logits, temperature, top_k, top_p)
        seq_prob_list.append(next_token_prob)
        ...
        prompt_ids = prompt_ids.to(seq.device)
        prompt_mask = prompt_mask.to(seq.device)
        return prompt_ids, prompt_mask, seq, seq_prob_list   
        # return seq, 1.0, time.time() - st
    
    @torch.no_grad()
    def decode_ids(self, ids, vq_model):
        bs = ids.shape[0]
        qzshape = [bs, 8, int(ids.shape[1]**0.5), int(ids.shape[1]**0.5)]
        quant_b, samples = vq_model.decode_code(ids, qzshape)
        return quant_b, samples
